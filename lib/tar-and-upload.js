...
function checkFilePath (filePrefix) {
  const readStream = fs.createReadStream(filePrefix)
  readStream.on('error', function () {
    throw new Error('Data is empty. Does file ' + filePrefix + ' exist?')
  })
  readStream.destroy()
}
...
function tarAndUpload (filePrefix, uploadUrl, authToken, opts, cb) {
  ...
  const stream = tar.pack(parentDirectory, {
    ...
    .on('data', function (e) {
      // Check for file path errors after we attempt to tar the file
      checkFilePath(filePrefix)
    })
